...
class TestFacultyAdviserCreateView(TestCase):
    ...
    def test_create(self):
        response = self.client.get(
            reverse('faculty_adviser_add'), data={'offer_proposition': self.education_group.pk}
        )
        self.assertEqual(response.status_code, HttpResponse.status_code)
        response = self.client.post(
            reverse('faculty_adviser_add')+'?offer_proposition={}'.format(self.education_group.pk),
            data={"adviser": self.adviser.pk}
        )
